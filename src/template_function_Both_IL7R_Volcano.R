...
Both_IL7R_Volcano <- function(DEG_Analysis) {
    # image: png
    ...
    df <- DEG_Analysis
    label.col <- "Gene"
    sig.col <- "Both-IL7R_pval"
    pCutoff  = 0.001
    lfc.col <- "Both-IL7R_logFC"
    FCcutoff = 1.0
...
#CHANGE LOG (NW 2022-03-22): Export significant counts to output log, remove from plot title (~line 160)
    ...
    png(
      filename="Both_IL7R_Volcano.png",
      width=imageWidth,
      height=imageHeight,
      units="px",
      pointsize=4,
...
print("template_function_Both_IL7R_Volcano.R #########################################################################")
...
var_Both_IL7R_Volcano<-Both_IL7R_Volcano(var_DEG_Analysis)
invisible(graphics.off())
saveRDS(var_Both_IL7R_Volcano, paste0(rds_output,"/var_Both_IL7R_Volcano.rds"))
