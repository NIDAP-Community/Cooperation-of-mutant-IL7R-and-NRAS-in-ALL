# Filter Low Counts [CCBR] (59e7ee62-1715-4276-b370-e8395168f9d8): v720
Filtered_Counts <- function(Clean_Raw_Counts, ccbr991_metadata_nidap) {
    ...
    #Basic Parameters:
    counts_matrix = Clean_Raw_Counts
    sample_metadata <- ccbr991_metadata_nidap   
    gene_names_column <- "Gene"
    columns_to_include <- c("Gene","control1","control2","control3","IL7R1","IL7R2","IL7R3","NRAS1","NRAS2","NRAS3","Both1","Both2","Both3")
...
var_Clean_Raw_Counts<-readRDS(paste0(rds_output,"/var_Clean_Raw_Counts.rds"))
Input_is_Seurat_count <- 0
for(item in var_Clean_Raw_Counts){ if (class(item)=="Seurat"){Input_is_Seurat_count = Input_is_Seurat_count + 1}}
if(Input_is_Seurat_count == 0 ){
var_Clean_Raw_Counts<-as.data.frame(var_Clean_Raw_Counts)}else{var_Clean_Raw_Counts <- var_Clean_Raw_Counts}
currentdir <- getwd()
rds_output <- paste0(currentdir,'/rds_output')
...
var_Filtered_Counts<-Filtered_Counts(var_Clean_Raw_Counts,var_ccbr991_metadata_nidap)
invisible(graphics.off())
