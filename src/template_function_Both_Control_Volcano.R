...
Both_Control_Volcano <- function(DEG_Analysis) {
    # image: png
    ...
    df <- DEG_Analysis
    label.col <- "Gene"
    sig.col <- "Both-Control_pval"
    pCutoff  = 0.001
    lfc.col <- "Both-Control_logFC"
    FCcutoff = 1.0
    ...
    additional_labels <- "Mcm4,Noc4l,Ung,Nolc1,Rrp9,Tcof1,Pprc1,Rrp12,Utp20,Plk1,Mcm5,Srm,Myc,Prmt3,Gnl3,Pa2g4,Nip7,Ddx18,Bysl,Nop56,Mrto4,Mybbp1a,Las1l,Wdr43,Hk2,Grwd1,Ipo4,Ndufaf4,Tmem97,Tbrg4,Farsa,Wdr74,Slc29a2,Mphosph10,Rcl1,Phb,Aimp2,Cdk4,Dctpp1,Dusp2,Exosc5,Hspe1,Imp4,Map3k6,Pes1,Plk4,Ppan,Pus1,Rabepk,Slc19a1,Sord,Supv3l1,Tfb2m"
    labSize <- 4    

    #Plot Parameters
    change_sig_name <- "p-value"
    change_lfc_name <- "log2FC"
    title <- "Both vs. Control"
    subtitle <- "p-value >= 0.001 & logFC > |1.0|"
...
#CHANGE LOG (NW 2022-03-22): Export significant counts to output log, remove from plot title (~line 160)
    ...
    png(
      filename="Both_Control_Volcano.png",
      width=imageWidth,
      height=imageHeight,
      units="px",
      pointsize=4,
...
print("template_function_Both_Control_Volcano.R #########################################################################")
...
var_Both_Control_Volcano<-Both_Control_Volcano(var_DEG_Analysis)
invisible(graphics.off())
saveRDS(var_Both_Control_Volcano, paste0(rds_output,"/var_Both_Control_Volcano.rds"))
