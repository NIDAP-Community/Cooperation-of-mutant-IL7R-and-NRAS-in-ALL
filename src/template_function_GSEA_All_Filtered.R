...
GSEA_All_Filtered <- function(DEG_Analysis, GSEA_Filtered, Batch_Corrected_Counts, ccbr991_metadata_nidap, msigdb_v6_2_with_orthologs) {
...
## PARAMETERS
    ...
    gex_df <- Batch_Corrected_Counts
    metadata_df <- ccbr991_metadata_nidap
    gsea_df <- GSEA_Filtered
...
if (grepl("LE", which_plot)) {
    ...
    samples_to_include = setdiff(c("control1","control2","control3","IL7R1","IL7R2","IL7R3","NRAS1","NRAS2","NRAS3","Both1","Both2","Both3"), geneid)
...
png(filename="GSEA_All_Filtered.png", width=(pageWidth+5)*300, height=(pageHeight+5)*300, units="px", pointsize=4, bg="white", res=300, type="cairo") 
print(preview)
...
print("template_function_GSEA_All_Filtered.R #########################################################################")
...
currentdir <- getwd()
rds_output <- paste0(currentdir,'/rds_output')
var_GSEA_Filtered<-readRDS(paste0(rds_output,"/var_GSEA_Filtered.rds"))
Input_is_Seurat_count <- 0
for(item in var_GSEA_Filtered){ if (class(item)=="Seurat"){Input_is_Seurat_count = Input_is_Seurat_count + 1}}
if(Input_is_Seurat_count == 0 ){
var_GSEA_Filtered<-as.data.frame(var_GSEA_Filtered)}else{var_GSEA_Filtered <- var_GSEA_Filtered}
currentdir <- getwd()
rds_output <- paste0(currentdir,'/rds_output')
...
var_GSEA_All_Filtered<-GSEA_All_Filtered(var_DEG_Analysis,var_GSEA_Filtered,var_Batch_Corrected_Counts,var_ccbr991_metadata_nidap,var_msigdb_v6_2_with_orthologs)
invisible(graphics.off())
saveRDS(var_GSEA_All_Filtered, paste0(rds_output,"/var_GSEA_All_Filtered.rds"))
