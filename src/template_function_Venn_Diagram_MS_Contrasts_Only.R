...
Venn_Diagram_MS_Contrasts_Only <- function(Volcano_Summary_MS_Contrasts_Only) {
...
# Basic parameters
input_dataset <- Volcano_Summary_MS_Contrasts_Only
...
if(image_output_format == 'png') {
    png(filename="Venn_Diagram_MS_Contrasts_Only.png", width=image_width, height=image_height, units="px", pointsize=4, bg="white", res=image_resolution, type="cairo")
} else {
    svglite::svglite(file="Venn_Diagram_MS_Contrasts_Only.png", width=round(image_width/image_resolution,digits=2), height=round(image_height/image_resolution,digits=2), pointsize=1, bg="white")
}
...
print("template_function_Venn_Diagram_MS_Contrasts_Only.R #########################################################################")
library(plotly);library(ggplot2);library(jsonlite);
currentdir <- getwd()
rds_output <- paste0(currentdir,'/rds_output')
var_Volcano_Summary_MS_Contrasts_Only<-readRDS(paste0(rds_output,"/var_Volcano_Summary_MS_Contrasts_Only.rds"))
Input_is_Seurat_count <- 0
for(item in var_Volcano_Summary_MS_Contrasts_Only){ if (class(item)=="Seurat"){Input_is_Seurat_count = Input_is_Seurat_count + 1}}
if(Input_is_Seurat_count == 0 ){
var_Volcano_Summary_MS_Contrasts_Only<-as.data.frame(var_Volcano_Summary_MS_Contrasts_Only)}else{var_Volcano_Summary_MS_Contrasts_Only <- var_Volcano_Summary_MS_Contrasts_Only}
invisible(graphics.off())
var_Venn_Diagram_MS_Contrasts_Only<-Venn_Diagram_MS_Contrasts_Only(var_Volcano_Summary_MS_Contrasts_Only)
invisible(graphics.off())
saveRDS(var_Venn_Diagram_MS_Contrasts_Only, paste0(rds_output,"/var_Venn_Diagram_MS_Contrasts_Only.rds"))
